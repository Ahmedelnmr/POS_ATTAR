...
<div class="pos-layout">
    ...
    <div class="pos-scanner-section">
        ...
        <div class="form-group">
            <label>رقم الفاتورة <small class="text-muted">(تلقائي)</small></label>
            <input type="text" id="invoiceNumber" class="form-control" placeholder="سيتم إنشاؤه تلقائياً" readonly style="background-color: #f8f9fa; cursor: not-allowed;">
        </div>
    ...
    <div class="pos-main-section">
        ...
        <div class="summary-card" style="background:#fff; border:1px solid #ddd; box-shadow:0 4px 6px rgba(0,0,0,0.05); color:#333; display: flex; flex-direction: column;">
            ...
            <div style="background:#f8f9fa; padding:10px; border-radius:6px; margin-bottom: 10px;">
                <label style="display:block; font-size:12px; margin-bottom:5px; color:#666;">الخصم</label>
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 5px; font-size: 12px;">
                    <span id="discountLabel" style="color:#666;">القيمة المخصومة:</span>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; padding-top:10px; border-top:2px solid #333; margin-top: auto;">
                <div style="font-size: 18px; font-weight: bold;">الإجمالي النهائي</div>
...
function updateTotals() {
    ...
    var discountValue = 0;
    var discountLabelText = 'القيمة المخصومة:';
    
    if(discountType === 'percent') {
        discountValue = (total * discountInput) / 100;
        discountLabelText = 'خصم (' + discountInput + '%):';
    } else {
        discountValue = discountInput;
        discountLabelText = 'خصم (مبلغ ثابت):';
    }
...
function renderItems() {
    ...
    if (invoiceItems.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted" style="padding:40px">امسح باركود أو ابحث لإضافة منتجات</td></tr>';
        updateTotals();
        return;
    }
    ...
    tbody.innerHTML = invoiceItems.map(function(item, i) {
        ...
        // Build SELECT dropdown (not datalist)
        var unitOptions = '';
        
        // Option 1: Unit (always available)
        var unitLabel = item.unit_name + ' - ' + item.db_unit_cost.toFixed(2) + ' ج.م';
        var unitSelected = item.unit_mode === 'unit' ? 'selected' : '';
        unitOptions += '<option value="unit" ' + unitSelected + '>' + unitLabel + '</option>';
        
        // Option 2: Pack (only if pack_qty > 1)
        if (item.pack_qty > 1) {
            var packLabel = item.pack_name + ' (' + item.pack_qty + ' ' + item.unit_name + ') - ' + item.db_pack_cost.toFixed(2) + ' ج.م';
            var packSelected = item.unit_mode === 'pack' ? 'selected' : '';
            unitOptions += '<option value="pack" ' + packSelected + '>' + packLabel + '</option>';
        }
        ...
        var bonusBadge = item.is_bonus ? '<span style="background:#28a745; color:white; padding:2px 6px; border-radius:4px; font-size:10px; margin-right:5px;">🎁 هدية</span>' : '';
        ...
        return '<tr style="' + typeStyle + '">' +
                ...
                '</td>' +
                '<td>' +
                    '<select class="unit-select" onchange="switchUnitMode(' + i + ', this.value)" style="width:140px; padding:4px;">' +
                        unitOptions +
                    '</select>' +
                '</td>' +
                '<td>' +
                ...
                '<td style="text-align:center;">' +
                    '<div style="display:flex; flex-direction:column; align-items:center; gap:2px;">' +
                        '<label style="font-size:10px; color:#666; margin:0;">هدية؟</label>' +
                        '<input type="checkbox" ' + (item.is_bonus ? 'checked' : '') + ' onchange="toggleBonus(' + i + ', this.checked)" title="منتج هدية (بونص)" style="width:18px; height:18px; cursor:pointer;">' +
                    '</div>' +
                '</td>' +
                '<td>' +
...
function switchUnitMode(index, mode) {
    var item = invoiceItems[index];
    
    console.log('🔄 Switching Unit Mode:', mode, 'for', item.product_name);
    ...
    if (mode === 'pack') {
        ...
        console.log('✅ Switched to PACK | Price:', item.purchase_price);
    } else {
        item.unit_mode = 'unit';
        ...
        console.log('✅ Switched to UNIT | Price:', item.purchase_price);
    }
